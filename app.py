...
def main():
	...
	if choice == "Home":
		...
		st.text("The aim is to develop an approach to find similar organisations based on their")
		st.text("profile description on Crunchbase. The short profile organizations’")
		st.text("descriptions tend to be focused on the core business model of a company and")
		st.text("the market they are addressing. The premise lies on the fact that if any two")
		st.text("organizations are operating within the same market space and doing the same") 
		st.text("things, they will use similar industry vocabulary to describe what they do.")
